...
- **王冠系统**（替代原型阶段的帽子）：
  ...
  - 王冠在宠物头上按 **7-6-5-4-3-2-1** 的金字塔堆叠：高阶王冠优先位于底层右侧，
    新获得的低阶王冠从更高层或当前层左侧出现，超出 28 顶后折叠成 `+N` 徽章；
...
- **多人房间**：
  ...
  - **公开 / 邀请制**：创建房间时可选择——公开房间上列表，任何人都能加入；邀请制房间
    不在列表出现，只有拿到代码的人能加入；
  - 加入后每 3s 上报宠物状态（昵称/token/王冠/活动相位/自定义宠物 URL/配色），所有人
  ...
  - **宠物场景**：房间成员会以**浮动宠物**出现在你的页面上、围绕你的宠物排列——以自己为
    锚点，支持**自由拖动 / 水平对齐 / 垂直对齐 / 网格排列 / 环绕排列**五种模式（宠物面板里
    一键切换，默认水平对齐、间距 24px；网格可设置行数与列数，无法无重叠放置的设置会被拒绝；
    自由位置本地记忆，刷新不丢）。所有宠物使用
    当前观察者的本地统一尺寸渲染，不会把尺寸同步给其他玩家；昵称与 token 信息默认始终
    显示，也可改为仅悬停显示；
...
- B：先在插件设置中确认使用与 A 相同的游戏服务器，再点宠物 → 房间列表里点 A 的公开房，
  或「用代码加入」只输入房间代码；
...
自动双客户端验证会修改 token 账本，必须只对两个隔离且可丢弃的 `DSH_HOME` 执行：

```sh
set DSG_VERIFY_DISPOSABLE=1
node tools/verify-multi.mjs
```
