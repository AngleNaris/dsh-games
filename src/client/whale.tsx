...
export interface PetVariant {
  id: string
  /** Locale key base for the variant name (custom variants reuse one key). */
  nameKey: string
  from: string
  to: string
}
...
export function isCustomVariant(id: string | undefined): boolean {
...
export function customVariantId(from: string, to: string): string {
...
export function petVariantOf(id: string | undefined): PetVariant {
  if (isCustomVariant(id)) {
    const [from, to] = (id as string).slice(CUSTOM_PREFIX.length).split(':')
    return {
      id: id as string,
...
export const DeepSeekWhale = memo(function DeepSeekWhale(props: {
  size: number
  style?: CSSProperties
  title?: string
  variant?: string
...
export const HAT_COLORS = [
  '#ff5a5f', '#ffb400', '#34c759', '#5ac8fa', '#af52de',
  '#ff2d55', '#ff9500', '#00c7be', '#ff375f', '#a2845e',
  '#64d2ff', '#bf5af2',
] as const

/** One party hat (cone + brim + pompom), drawn for a hat cell of `size` px. */
export function PartyHat(props: { color: string; size: number; style?: CSSProperties }): ReactElement {
  const { color, size, style } = props
  return (
    <svg viewBox="0 0 40 40" width={size} height={size} style={style} aria-hidden>
      <defs>
        <linearGradient id={`hat-${color.slice(1)}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={color} />
          <stop offset="100%" stopColor={shade(color, -18)} />
        </linearGradient>
      </defs>
      <circle cx="20" cy="5.5" r="5" fill={shade(color, 26)} />
      <path d="M20 5 C22 12 28 24 32.5 30.5 C26 33 14 33 7.5 30.5 C12 24 18 12 20 5 Z" fill={`url(#hat-${color.slice(1)})`} />
      <ellipse cx="20" cy="31" rx="16.5" ry="3.6" fill={shade(color, -28)} />
      <path d="M9 29 C13 26.5 27 26.5 31 29 C27 31.5 13 31.5 9 29 Z" fill={shade(color, 14)} opacity="0.55" />
    </svg>
  )
}
...
function shade(hex: string, delta: number): string {
  ...
  const r = clamp256(((num >> 16) & 0xff) + delta)
  const g = clamp256(((num >> 8) & 0xff) + delta)
  const b = clamp256((num & 0xff) + delta)
  return `#${((r << 16) | (g << 8) | b).toString(16).padStart(6, '0')}`
}

function clamp256(v: number): number {
  return Math.min(255, Math.max(0, v))
}

/** How many hats to actually render (the rest collapse into a badge). */
export const MAX_RENDERED_HATS = 12

/**
 * Hats stacked in a shallow arc above the whale. Returns positioned hat
 * elements plus the overflow count.
 */
export function renderHats(count: number, hatSize: number): { hats: ReactElement[]; overflow: number } {
  const shown = Math.min(count, MAX_RENDERED_HATS)
  const overflow = count - shown
  const hats: ReactElement[] = []
  for (let i = 0; i < shown; i += 1) {
    const x = (i - (shown - 1) / 2) * hatSize * 0.62
    const y = -hatSize * 0.5 - Math.abs(x) * 0.28
    const rotation = (i - (shown - 1) / 2) * -7
    hats.push(
      <div
        key={i}
        className="dsg-hat"
        style={{
          ...
          left: '50%',
          top: 0,
          transform: `translate(calc(-50% + ${x.toFixed(1)}px), ${y.toFixed(1)}px) rotate(${rotation.toFixed(1)}deg)`,
        }}
      >
